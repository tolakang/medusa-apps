...
- [ ] P0-19 Independence proof: redeploy each app alone, then roll one back. — in progress. Step 1 done 2026-09-26: backend deployed alone (container start 12:21 UTC), storefront alone (12:32), worker alone (12:46:31, worker mode, Redis modules connected, no errors). After each deploy the other apps' logs showed no new startup, and backend `/health` + storefront `/dk` returned 200 on 116/116 checks during the worker deploy (12:43:58 to about 12:51). Worker Swarm Update Config set (`stop-first`, rollback). Step 2 first run (PR #4, merged 12:56:14 UTC; changed `ecommerce/storefront/Dockerfile` + docs outside all watch paths): storefront auto-deployed (ready 12:58:15) ✅, backend did not (still 12:21:34) ✅, **worker did (13:01:35)** ❌ because its Watch Paths box was empty (F-019). Backend `/health` + storefront `/dk` 200 on 247/247 checks (12:52 to 13:07). Worker Watch Paths set to `ecommerce/backend/**`. Step 2 second run (PR #5 `a0be446`, merged 13:11:09 UTC; changed `ecommerce/storefront/README.md` + docs): storefront auto-deployed (ready 13:13:19) ✅, backend did not (its Deployments list has no entry for `96db147` or `a0be446`) ✅, **worker did again** (Deployments row 2 = `a0be446`, container `s68f…` started 13:16:33 after `stop-first` stopped `y37m…` at 13:16:29) ❌, although its Watch Paths showed `ecommerce/backend/**` after a reload. Operator then deployed the worker manually (container `z0xi…`, ready 13:24:48, healthy). Cause not proven: the save may have landed after the push, or the worker's config differs. Third run (PR #6 `8345a0d`, merged 13:30:33 UTC; changed only `TASKS.md`): **no app deployed** (no `8345a0d` row in the backend, worker or storefront Deployments lists, checked ~13:35, 5 min after the merge, while the worker's earlier push deploys started ~2 min after their merges) ✅. Conclusion: the worker's Watch Paths work once saved; the PR #5 redeploy most likely came from the save landing after that push. **Step 2 passed** (storefront-only pushes deploy only the storefront: PR #4 apart from the then-empty worker box; non-watched push deploys nothing: PR #6). **Step 3 (rollback) postponed 2026-09-26 by the operator:** no shell on the Dokploy server yet, and without one the rollback can't be proven (F-020). P0-19 = 2 of 3 steps done; step 3 needs `docker service inspect`/`docker service ps` access (runbook section 6.3).
...
| F-019 | 2026-09-26 | 5 (Dokploy) | The staging worker's Watch Paths box was empty, so it redeployed on a push that touched only `ecommerce/storefront/**` and docs. The backend (`ecommerce/backend/**`) and storefront (`ecommerce/storefront/**`) filtered correctly. | P0-19 step 2: PR #4 merge 12:56:14 UTC → worker restart 13:01:35, backend unchanged (12:21:34); operator read all three Watch Paths boxes | Worker set to `ecommerce/backend/**`; runbook section 3 now names the value and warns about an empty box. The PR #5 push still redeployed the worker (Deployments row `a0be446`); the PR #6 push (only `TASKS.md`) deployed nothing, so the saved setting works; the PR #5 redeploy most likely came from the save landing after that push. Resolved. | Informational |
| F-020 | 2026-09-26 | 5 (Dokploy) | Dokploy's Swarm Settings are a **form** in this version (the runbook assumed JSON boxes). Update Config was never set on backend or storefront. After entering Failure Action **Rollback** and Order **Start First**, Dokploy showed "Update config updated successfully", but on reopening both dropdowns were blank again while the numbers (Parallelism 1, Delay/Monitor 10 s, Max Failure Ratio 0) stayed. Whether the dropdown values are stored is unknown. Deploys already behave start-first (0 failed checks at 2 to 3 s intervals across four redeploys). Upstream: the Dokploy UI may report a rolled-back Swarm deploy as successful (Dokploy/dokploy#3987). | Operator screenshots of Update Config before/after save; external watches 177/247/329/212 checks with 0 failures | Runbook: form-field values and a server-side check (`docker service inspect … .Spec.UpdateConfig`). P0-19 step 3 postponed until there is shell access. | Operator (tolakang) chose to postpone step 3, 2026-09-26 |
