...
**Deploy · plan §5**
- [~] P0-18 Dokploy staging project: Postgres, Redis, backend server, worker, storefront. Build paths, watch paths, health check `/health`, first-deploy order. — in progress: `ecommerce-staging` backend live at api2.nokor24.com (`/health` 200, `/app/` 200, store w/o key 400, admin w/o login 401, CORS shop2 allowed / foreign refused); admin user + staging seed done; worker and storefront pending
...
| F-017 | 2026-09-25 | 5 | The storefront was built with Medusa's auto-created default key (F-005), which sees **0** products, so product pages 404. | `GET /store/products` with that key → `count: 0` | Runbook check added; use the seed's Default Sales Channel key. | Informational |
